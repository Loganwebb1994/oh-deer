...
export const StandCard = ({ stand, relationships }) => {
  ...
  const renderNoteButtons = () => {
    return(
      ...
      <button className="stand__notes__edit" onClick={() => {history.push(`/stands/${stand.id}`)}} id= {stand.id}>Edit Note</button>
      <button className="stand__notes__delete" onClick={() => addNote({note:""}, userStandId )}>Delete Note</button>
      <button className="stand__addNote" onClick={() => { history.push(`/stands/${stand.id}`) }}>Add Note</button>
      </>
    )
